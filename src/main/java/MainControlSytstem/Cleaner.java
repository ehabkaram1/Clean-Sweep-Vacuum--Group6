...
public class Cleaner {
  ...
  public double calculateUnit() {
    ...
    while (index != map.get(0)) {
      // averagePowerCost = (this.prevNode.getBatteryConsumption() +
      // this.currNode.getBatteryConsumption()) / 2;
      unitNum =
          unitNum + ((index.getBatteryConsumption() + index.parent.getBatteryConsumption()) / 2);
      index = index.parent;
    }
    return unitNum;
  }
  ...
  private void teleportToNode(FloorTile node, boolean shouldClean) {
    ...
    if (this.currNode.getChargeStation()) {
      ...
      System.out.println("************************");
      System.out.println("The battery is recharged fully.");
      System.out.println("************************");
  ...
  public boolean moveAhead() {
    ...
    currentMap[this.currNode._x][this.currNode._y] =
        copyFloorTile(this.currNode); // copy sensor node to the cleaner's map
  ...
  // For moving left, right or back, the heading direction will change correspondingly, and then
  // move forward.
  ...
  public void cleanSurface() {
    ...
    if (checkMapCleaningComplete()) {
      ...
      if (closest != null) {
        ...
        int y = closest.get_y();
        System.out.println("***************");
        System.out.println("The cleaning is now completed.");
        System.out.println("***************");
